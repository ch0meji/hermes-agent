...
def _patch_quickstart_preflight(monkeypatch):
    """Keep success-path tests independent of the host's real hardware budget."""
...
@pytest.fixture
def quickstart_target_ready(monkeypatch):
    _patch_quickstart_preflight(monkeypatch)


def test_quickstart_runs_all_three_legs(client, quickstart_target_ready, monkeypatch, tmp_path):
...
def test_quickstart_skips_satisfied_legs(client, quickstart_target_ready, monkeypatch):
