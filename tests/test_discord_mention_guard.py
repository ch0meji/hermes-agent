...
from plugins.platforms.discord.mention_guard import raw_mentioned_user_ids, targets_other_bot_only
...
def test_hatsugarasu_only_mention_is_for_nashi_to_ignore():
...
def test_message_explicitly_mentioning_both_bots_is_not_blocked():
    assert targets_other_bot_only(
        message(f"<@{HATSUGARASU_ID}> <@{NASHI_ID}> 2人とも確認して"),
        other_bot_user_id=HATSUGARASU_ID,
        self_bot_user_id=NASHI_ID,
    ) is False
